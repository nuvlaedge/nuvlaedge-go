...
func Test_NewCommissioner(t *testing.T) {
	// Test function for NewCommissioner
	c := NewCommissioner(1, nil, nil)
	assert.NotNil(t, c)
	assert.Equal(t, 1, c.Period)
	assert.Equal(t, DefaultTags, c.currentData.Tags)
	assert.Equal(t, DefaultCapabilities, c.currentData.Capabilities)
}
...
func Test_Commissioner_GetType(t *testing.T) {
	c := NewCommissioner(1, nil, nil)
	assert.Equal(t, types.Commissioner, c.GetType())
}
